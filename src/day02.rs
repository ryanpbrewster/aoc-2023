...
pub fn part1(input: &str) -> anyhow::Result<usize> {
    let filter: BTreeMap<Color, u32> = [(Color::Red, 12), (Color::Green, 13), (Color::Blue, 14)]
        .into_iter()
        .collect();
    let mut total = 0;
    for line in input.trim().lines().map(str::trim) {
        let g = parse_game(line)?;
        if g.rounds.into_iter().all(|round| {
            round
                .piles
                .into_iter()
                .all(|(color, count)| count <= filter.get(&color).copied().unwrap_or_default())
        }) {
            total += g.id;
        }
    }
...
pub fn part2(input: &str) -> anyhow::Result<u32> {
    ...
    for line in input.trim().lines().map(str::trim) {
        let g = parse_game(line)?;
        let mut caps: BTreeMap<Color, u32> = BTreeMap::new();
        for round in g.rounds {
            for (color, count) in round.piles {
                let cur = caps.entry(color).or_default();
                if count > *cur {
                    *cur = count;
                }
            }
        }
        total += caps.values().copied().product::<u32>();
    }
    Ok(total)
}
...
fn game_parser(input: &str) -> IResult<&str, Game> {
    let (input, (id, rounds)) = separated_pair(game_id_parser, tag(": "), rounds_parser)(input)?;
    Ok((input, Game { id, rounds }))
}

// "Game 5" -> 5
fn game_id_parser(input: &str) -> IResult<&str, usize> {
    let (input, id) = preceded(tag("Game "), map_res(digit1, str::parse))(input)?;
    Ok((input, id))
}

// "4 green; 2 red, 4 blue" -> { [(Color::Green, 4)]; [(Color::Red, 2), (Color::Blue, 4)] }
fn rounds_parser(input: &str) -> IResult<&str, Vec<Round>> {
    separated_list1(tag("; "), round_parser)(input)
}

// "2 red, 4 blue" -> [(Color::Red, 2), (Color::Blue, 4)]
fn round_parser(input: &str) -> IResult<&str, Round> {
    let (input, piles) = separated_list1(tag(", "), pile_parser)(input)?;

    Ok((
        input,
        Round {
            piles: piles.into_iter().collect(),
        },
    ))
}

// 4 green -> (Color::Green, 3)
fn pile_parser(input: &str) -> IResult<&str, (Color, u32)> {
    let (input, (count, color)) =
        separated_pair(map_res(digit1, str::parse), multispace1, color_parser)(input)?;
...
mod test {
    ...
    fn part1_parser() {
        ...
        assert_eq!(
            ...
            Game {
                ...
                rounds: vec![
                    Round {
                        piles: [(Color::Blue, 3), (Color::Red, 4)].into_iter().collect()
                    },
                    Round {
                        piles: [(Color::Red, 1), (Color::Green, 2), (Color::Blue, 6)]
                            .into_iter()
                            .collect()
                    },
                    Round {
                        piles: [(Color::Green, 2)].into_iter().collect()
                    },
                ]
            },
        );
    ...
    fn part1_real_input() {
        ...
        assert_eq!(part1(&input).unwrap(), 1867);
    }
    ...
    fn part2_real_input() {
        let input = std::fs::read_to_string("data/day02.input").unwrap();
        assert_eq!(part2(&input).unwrap(), 84538);
    }
}
